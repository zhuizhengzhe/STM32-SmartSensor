...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSwitch1)
    {
        info->event[info->num] = EVENT_Switch1;
        info->num++;
        dataPoints->valueSwitch1 = gizStandardDecompressionValue(Switch1_BYTEOFFSET,Switch1_BITOFFSET,Switch1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch2)
    {
        info->event[info->num] = EVENT_Switch2;
        info->num++;
        dataPoints->valueSwitch2 = gizStandardDecompressionValue(Switch2_BYTEOFFSET,Switch2_BITOFFSET,Switch2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch3)
    {
        info->event[info->num] = EVENT_Switch3;
        info->num++;
        dataPoints->valueSwitch3 = gizStandardDecompressionValue(Switch3_BYTEOFFSET,Switch3_BITOFFSET,Switch3_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch4)
    {
        info->event[info->num] = EVENT_Switch4;
        info->num++;
        dataPoints->valueSwitch4 = gizStandardDecompressionValue(Switch4_BYTEOFFSET,Switch4_BITOFFSET,Switch4_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch5)
    {
        info->event[info->num] = EVENT_Switch5;
        info->num++;
        dataPoints->valueSwitch5 = gizStandardDecompressionValue(Switch5_BYTEOFFSET,Switch5_BITOFFSET,Switch5_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch6)
    {
        info->event[info->num] = EVENT_Switch6;
        info->num++;
        dataPoints->valueSwitch6 = gizStandardDecompressionValue(Switch6_BYTEOFFSET,Switch6_BITOFFSET,Switch6_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSwitch7)
    {
        info->event[info->num] = EVENT_Switch7;
        info->num++;
        dataPoints->valueSwitch7 = gizStandardDecompressionValue(Switch7_BYTEOFFSET,Switch7_BITOFFSET,Switch7_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSwitch1 != cur->valueSwitch1)
    {
        GIZWITS_LOG("valueSwitch1 Changed\n");
        ret = 1;
    }
    if(last->valueSwitch2 != cur->valueSwitch2)
    {
        GIZWITS_LOG("valueSwitch2 Changed\n");
        ret = 1;
    }
    if(last->valueSwitch3 != cur->valueSwitch3)
    {
        GIZWITS_LOG("valueSwitch3 Changed\n");
        ret = 1;
    }
    if(last->valueSwitch4 != cur->valueSwitch4)
    {
        GIZWITS_LOG("valueSwitch4 Changed\n");
        ret = 1;
    }
    if(last->valueSwitch5 != cur->valueSwitch5)
    {
        GIZWITS_LOG("valueSwitch5 Changed\n");
        ret = 1;
    }
    if(last->valueSwitch6 != cur->valueSwitch6)
    {
        GIZWITS_LOG("valueSwitch6 Changed\n");
        ret = 1;
    }
    if(last->valueSwitch7 != cur->valueSwitch7)
    {
        GIZWITS_LOG("valueSwitch7 Changed\n");
        ret = 1;
    }

    if(last->valueGas_Concentration != cur->valueGas_Concentration)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueGas_Concentration Changed\n");
            ret = 1;
        }
    }
    if(last->valueInput_Voltage != cur->valueInput_Voltage)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueInput_Voltage Changed\n");
            ret = 1;
        }
    }
    if(last->valueCollection_Voltage != cur->valueCollection_Voltage)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCollection_Voltage Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Switch1_BYTEOFFSET,Switch1_BITOFFSET,Switch1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch1);
    gizStandardCompressValue(Switch2_BYTEOFFSET,Switch2_BITOFFSET,Switch2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch2);
    gizStandardCompressValue(Switch3_BYTEOFFSET,Switch3_BITOFFSET,Switch3_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch3);
    gizStandardCompressValue(Switch4_BYTEOFFSET,Switch4_BITOFFSET,Switch4_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch4);
    gizStandardCompressValue(Switch5_BYTEOFFSET,Switch5_BITOFFSET,Switch5_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch5);
    gizStandardCompressValue(Switch6_BYTEOFFSET,Switch6_BITOFFSET,Switch6_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch6);
    gizStandardCompressValue(Switch7_BYTEOFFSET,Switch7_BITOFFSET,Switch7_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSwitch7);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueGas_Concentration = gizY2X(Gas_Concentration_RATIO,  Gas_Concentration_ADDITION, dataPoints->valueGas_Concentration); 

    devStatusPtr->valueInput_Voltage = exchangeBytes(gizY2XFloat(Input_Voltage_RATIO,  Input_Voltage_ADDITION, dataPoints->valueInput_Voltage));     
    devStatusPtr->valueCollection_Voltage = exchangeBytes(gizY2XFloat(Collection_Voltage_RATIO,  Collection_Voltage_ADDITION, dataPoints->valueCollection_Voltage));     
